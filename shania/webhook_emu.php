		...
		foreach($_POST['events'] as $event){
			...
			if($source['type'] == 'user'){
				...
				if($event['type'] == 'follow'){
					...
					$messages = process_messages($app->on_message($event['message']['text'],$app->on_message($event['message'],$event));
				}else if($event['type'] == 'postback'){
					$messages = process_messages($app->on_postback($event['postback']['data'],$event['postback'],$event));
